...
GITHUB_TOKEN = "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"
...
def fetch_packages(org_or_user):
    ...
    response = requests.get(
        f"https://api.github.com/users/{org_or_user}/packages", headers=HEADERS
    )
    print(response.json())
...
def fetch_package_versions(org_or_user, package_name):
    ...
    response = requests.get(
        f"https://api.github.com/repos/{org_or_user}/packages/container/{package_name}/versions",
        headers=HEADERS,
    )
    if response.status_code != 200:
